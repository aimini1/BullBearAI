...
def fetch_option_chain(underlying_key, expiry="current_week"):
    """Fetch live CE/PE option-chain data from Upstox."""
    token = os.getenv("UPSTOX_ACCESS_TOKEN")
    if not token:
        return {"status": "error", "message": "UPSTOX_ACCESS_TOKEN is not configured"}
    ...
    try:
        ...
        for row in rows:
            ...
            cleaned.append({
                ...
                "call": {
                    ...
                    "ltp": cm.get("ltp"),
                    "volume": cm.get("volume"),
                    "oi": call_oi,
                    "prev_oi": cm.get("prev_oi"),
                    "change_oi": (
                        float(call_oi) - float(cm["prev_oi"])
                        if call_oi is not None and cm.get("prev_oi") is not None else None
                    ),
                ...
                "put": {
                    ...
                    "oi": put_oi,
                    "prev_oi": pm.get("prev_oi"),
                    "change_oi": (
                        float(put_oi) - float(pm["prev_oi"])
                        if put_oi is not None and pm.get("prev_oi") is not None else None
                    ),
                    "iv": (put.get("option_greeks") or {}).get("iv"),
                },
                "pcr_oi": pcr,
            })
        ...
        return {"status": "error", "message": f"Upstox option-chain HTTP error: {exc}"}
    except Exception as exc:
        return {"status": "error", "message": f"Option-chain error: {exc}"}


@app.get("/api/options")
def options_data(underlying: str = "NIFTY", expiry: str = "current_week"):
    keys = {
        "NIFTY": "NSE_INDEX|Nifty 50",
        "BANKNIFTY": "NSE_INDEX|Nifty Bank",
    }
    key = keys.get(underlying.upper())
    if not key:
        return {"status": "error", "message": "Use underlying=NIFTY or BANKNIFTY"}

    return fetch_option_chain(key, expiry)


@app.get("/api/options/summary")
def option_chain_summary(underlying: str = "NIFTY", expiry: str = "current_week"):
    result = options_data(underlying, expiry)
    if result.get("status") != "success":
        return result

    rows = result.get("data", [])
    spot = next((r.get("spot") for r in rows if r.get("spot") is not None), None)
    atm = None
    if spot is not None and rows:
        atm = min(rows, key=lambda r: abs(float(r["strike"]) - float(spot))).get("strike")

    near = [r for r in rows if atm is not None and abs(float(r["strike"]) - float(atm)) <= 5 * (
        abs(float(rows[1]["strike"]) - float(rows[0]["strike"])) if len(rows) > 1 else 50
    )]

    call_oi = sum((r["call"].get("oi") or 0) for r in near)
    put_oi = sum((r["put"].get("oi") or 0) for r in near)
    ...
    return {
        ...
        "spot": spot,
        "atm_strike": atm,
        "near_atm_strikes": len(near),
        "near_atm_call_oi": call_oi,
        "near_atm_put_oi": put_oi,
        "near_atm_pcr": pcr,
        "note": "Descriptive market data only; no trading recommendation.",
    }
