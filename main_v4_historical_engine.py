...
def fetch_historical(key, interval, days_back=5):
    ...
    url = (
        f"{UPSTOX}/v3/historical-candle/{quote(key, safe='')}/"
        f"minutes/{interval}/{to_date}/{from_date}"
    )
    ...
    try:
        ...
        if response.ok:
            ...
            return parse_rows(data.get("candles"))
    except Exception as exc:
        state["error"] = f"historical_candle_error: {exc}"
    return []


def merge_rows(*sets, limit=1000):
    by_time = {}
    for rows in sets:
        for row in rows:
...
def calculate_analysis(name):
    ...
    if len(rows) < 50:
        ...
        analysis[name] = {
            ...
            "timeframe": "5m",
            "candles": len(rows),
            "needed_for_ema50": max(0, 50 - len(rows)),
            "bias": "WAITING",
            "confidence": 0,
            ...
            "note": "Collecting enough 5-minute candles for EMA50.",
        }
        return
    ...
    # Index candles may have zero volume. Never invent a VWAP in that case.
    if vw is not None:
        max_score += 1
        if current > vw:
            score += 1
        ...
        reasons.append("VWAP unavailable: index volume is zero")
    ...
    if max_score == 0:
        ...
        if normalized >= 0.35:
            ...
            bias = "NEUTRAL"
        confidence = round(min(95, 50 + abs(normalized) * 45))
    ...
    while True:
        try:
            ...
            for name, key in INSTRUMENTS.items():
                ...
                hist5 = fetch_historical(key, 5, days_back=5)

                if one:
                    candles[name]["1m"] = merge_rows(candles[name]["1m"], one, limit=1000)
                    got_any = True

                merged5 = merge_rows(hist5, five, limit=1000)
                if merged5:
                    candles[name]["5m"] = merged5
                    got_any = True
    ...
    return {
        ...
        "checks": {
            name: {
                ...
                "ema20_ready": analysis[name].get("ema20") is not None,
                "ema50_ready": analysis[name].get("ema50") is not None,
                "vwap_available": analysis[name].get("vwap_available", False),
                "rsi14_ready": analysis[name].get("rsi14") is not None,
                "bias": analysis[name].get("bias"),
            }
...
async function refresh(){
 try{
  ...
  document.getElementById('engine').textContent=Object.values(d.analysis||{}).some(x=>x.status==='ready')
   ? 'EMA20/50 • VWAP (when volume exists) • RSI14 • Market Structure • Bias ACTIVE'
   : 'Collecting enough 5-minute candles…';
