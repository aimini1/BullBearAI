...
def fetch_option_chain(name, expiry="current_week"):
    ...
    while True:
        try:
            ...
            for option_name in OPTIONS_UNDERLYINGS:
                fetch_option_chain(option_name, "current_week")
...
def options_chain_api(underlying: str):
    ...
    return option_chain_snapshot(name, strikes_each_side=5)
...
def options_chain_all():
    return {
        ...
        "expiry": "current_week",
        "underlyings": {
            name: option_chain_snapshot(name, strikes_each_side=5)
            for name in OPTIONS_UNDERLYINGS
        },
    }
...
<div class="label">OPTION CHAIN • CURRENT WEEK</div>
...
async function refresh(){
 try{
  ...
  Object.entries(opt.underlyings||{}).forEach(([name,o])=>{
    const rows=o.data||[];
    html+=`<div class="section"><b>${name}</b> • Spot ${fmt(o.spot)} • ATM ${fmt(o.atm_strike)} • ${o.status}`;
