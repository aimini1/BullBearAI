...
def pwa_manifest():
    return JSONResponse({
        ...
        "theme_color": "#0b1220",
        "description": "BullBear AI live Indian market analytics dashboard",
...
def pwa_service_worker():
    return Response(
        """const CACHE_NAME='bullbear-ai-pwa-v2';
...
function renderNews(d){
  ...
  const items=d.items||[];
  $('newsContent').innerHTML=items.length?items.map(n=>`<div class="news-item">
    <a class="news-link" href="${esc(n.link)}" target="_blank" rel="noopener"><div class="news-title">${esc(n.title)}</div></a>
    <div class="news-meta">${esc(n.source||'Market News')} • ${esc(n.published||'')}</div>
  </div>`).join(''):`<div class="signal-meta">${esc(d.error||'Waiting for market news…')}</div>`;
}
...
async function refresh(){
 try{
  ...
  const news=await (await fetch('/api/news?ts='+Date.now(),{cache:'no-store'})).json();renderNews(news);
