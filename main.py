...
# Dedicated chart-analysis state. It is intentionally descriptive: support/resistance
# levels are derived from historical candles, while break/rejection percentages are
# heuristic estimates from historical tests + current candle/volume/liquidity context.
chart_analysis = {name: {"status": "waiting", "timeframe": "5m", "candles": [], "levels": [], "context": {}, "updated_at": None, "error": None} for name in INSTRUMENTS}
CHART_ANALYSIS_REFRESH_SECONDS = 60
chart_analysis_at = {name: None for name in INSTRUMENTS}
...
def _candle_pattern(rows):
    ...
    if body <= rng * 0.12:
        return "DOJI"
    if (b["close"] > b["open"] and a["close"] < a["open"] and
...
def _chart_pattern(rows):
    if len(rows) < 12:
        return "INSUFFICIENT DATA"
    recent = rows[-12:]
    highs = [r["high"] for r in recent]
    lows = [r["low"] for r in recent]
    h1, h2 = max(highs[:6]), max(highs[6:])
    l1, l2 = min(lows[:6]), min(lows[6:])
    if h2 > h1 and l2 > l1:
        return "HIGHER HIGH + HIGHER LOW"
    if h2 < h1 and l2 < l1:
        return "LOWER HIGH + LOWER LOW"
    # Simple double-top / double-bottom proximity pattern.
    tol = max(recent[-1]["close"] * 0.0015, 1.0)
    if abs(h1 - h2) <= tol:
        return "DOUBLE-TOP CANDIDATE"
    if abs(l1 - l2) <= tol:
        return "DOUBLE-BOTTOM CANDIDATE"
    return "RANGE / MIXED"


def _volume_ratio(rows, window=20):
    vols = [float(r.get("volume") or 0) for r in rows[-window:]]
    if len(vols) < 5:
        return None
...
def _levels_from_history(rows):
    """Find clustered local swing highs/lows from 5M historical candles."""
    if len(rows) < 30:
        return []
    closes = [r["close"] for r in rows]
    tolerance = max((max(closes[-300:]) - min(closes[-300:])) * 0.003, closes[-1] * 0.0008)
    candidates = []
    look = 2
    for i in range(look, len(rows) - look):
        r = rows[i]
        left = rows[i-look:i]
        right = rows[i+1:i+look+1]
        if r["high"] >= max(x["high"] for x in left + right):
            candidates.append(("resistance", r["high"], i))
        if r["low"] <= min(x["low"] for x in left + right):
            candidates.append(("support", r["low"], i))
    clusters = []
    for typ, price, idx in sorted(candidates, key=lambda x: x[1]):
        found = None
        for c in clusters:
            if c["type"] == typ and abs(c["price"] - price) <= tolerance:
                c["prices"].append(price); c["indices"].append(idx)
                c["price"] = sum(c["prices"]) / len(c["prices"])
                found = c
                break
        if found is None:
            clusters.append({"type": typ, "price": price, "prices": [price], "indices": [idx]})
    current = closes[-1]
    for c in clusters:
        c["touches"] = len(c["prices"])
        c["distance_pct"] = abs(current - c["price"]) / current * 100
        # Historical test: after a touch, did price close through the level within 3 candles?
        tests = breaks = rejections = 0
        level = c["price"]
        tol = tolerance
        for idx in c["indices"]:
            if idx + 3 >= len(rows):
                continue
            window = rows[idx+1:idx+4]
            tests += 1
            if c["type"] == "resistance":
                broke = any(x["close"] > level + tol for x in window)
            else:
                broke = any(x["close"] < level - tol for x in window)
            if broke:
                breaks += 1
                ...
                rejections += 1
        c["tests"] = tests
        c["historical_break_pct"] = round((breaks / tests) * 100, 1) if tests else None
        c["historical_rejection_pct"] = round((rejections / tests) * 100, 1) if tests else None
    # Keep the most meaningful nearby levels.
    clusters.sort(key=lambda c: (c["distance_pct"], -c["touches"]))
    return clusters[:8]


def build_chart_analysis(name):
    rows = candles[name]["5m"]
    if len(rows) < 30:
        chart_analysis[name].update(status="waiting", error="Need at least 30 historical 5M candles", candles=rows[-120:])
        return
    recent = rows[-120:]
    current = recent[-1]["close"]
    vr = _volume_ratio(recent)
    candle = _candle_pattern(recent)
    pattern = _chart_pattern(recent)
    levels = _levels_from_history(rows)
    liquidity_score = 50.0
    if vr is not None:
        liquidity_score += min(25.0, max(-20.0, (vr - 1.0) * 20.0))
    last_range = max(recent[-1]["high"] - recent[-1]["low"], 1e-9)
    wick_ratio = last_range / max(abs(recent[-1]["close"] - recent[-1]["open"]), last_range * 0.1)
    if wick_ratio >= 3.0: liquidity_score -= 5.0
    liquidity_score = round(max(0.0, min(100.0, liquidity_score)), 1)
    # Current context score: positive means pressure toward upside break; negative toward downside.
    context_score = 0.0
    if recent[-1]["close"] > recent[-1]["open"]: context_score += 1.0
    else: context_score -= 1.0
    if vr is not None and vr >= 1.5: context_score += 1.0 if recent[-1]["close"] > recent[-1]["open"] else -1.0
    if candle in ("BULLISH ENGULFING", "HAMMER"): context_score += 1.0
    if candle in ("BEARISH ENGULFING", "SHOOTING STAR"): context_score -= 1.0
    if pattern == "HIGHER HIGH + HIGHER LOW": context_score += 1.0
    elif pattern == "LOWER HIGH + LOWER LOW": context_score -= 1.0

    enriched = []
    for c in levels:
        # Only levels near the current market are useful for the dedicated chart page.
        base = c.get("historical_break_pct")
        if base is None:
            base = 50.0
        directional = context_score * 4.0
        if c["type"] == "support":
            # Bullish context tends to defend support; bearish context increases break risk.
            estimated_break = max(5.0, min(95.0, base - directional))
        else:
            # Bullish context increases resistance-break risk.
            estimated_break = max(5.0, min(95.0, base + directional))
        estimated_hold = 100.0 - estimated_break
        e = dict(c)
        e["estimated_break_pct"] = round(estimated_break, 1)
        e["estimated_hold_pct"] = round(estimated_hold, 1)
        e["role"] = "nearest" if len(enriched) < 2 else "historical"
        enriched.append(e)
    enriched.sort(key=lambda c: c["distance_pct"])
    chart_analysis[name] = {
        "status": "ready", "timeframe": "5m", "price": round(current, 2),
        "candles": recent, "levels": enriched,
        "context": {"candle_pattern": candle, "chart_pattern": pattern,
                     "volume_ratio": round(vr, 2) if vr is not None else None,
                     "liquidity_proxy": "volume + wick/sweep proxy (index order-book unavailable)",
                     "liquidity_score": liquidity_score,
                     "context_score": round(context_score, 2)},
        "updated_at": now_ist(), "error": None,
        "note": "Break/hold percentages are heuristic estimates, not guaranteed probabilities or trading recommendations."
    }


def refresh_chart_analysis_if_due(name, now):
    last = chart_analysis_at.get(name)
    age = (now - last).total_seconds() if last else 10**9
    if age >= CHART_ANALYSIS_REFRESH_SECONDS or chart_analysis[name].get("status") != "ready":
        build_chart_analysis(name)
        chart_analysis_at[name] = now
        ...
        try:
            ...
            for name, key in INSTRUMENTS.items():
                ...
                if not historical_cache[name] or cache_age >= 300:
                    hist5 = fetch_historical_candles(key, 5, days=30)
                    if hist5:
                ...
                refresh_chart_analysis_if_due(name, now)
...
@app.get("/api/chart-analysis/{underlying}")
def chart_analysis_api(underlying: str):
    name = underlying.strip().upper()
    aliases = {"NIFTY": "NIFTY 50", "NIFTY 50": "NIFTY 50", "BANKNIFTY": "BANK NIFTY", "BANK NIFTY": "BANK NIFTY", "VIX": "INDIA VIX", "INDIA VIX": "INDIA VIX"}
    name = aliases.get(name)
    if name not in INSTRUMENTS:
        return {"status": "error", "detail": "Use NIFTY 50, BANK NIFTY or INDIA VIX"}
    return chart_analysis.get(name, {"status": "waiting"})
...
.chart{height:190px;background:#07101a;border:1px solid var(--line);border-radius:7px;overflow:hidden;position:relative}.chart svg{width:100%;height:100%}.chartlabel{position:absolute;left:7px;top:6px;color:var(--muted);font-size:8px}.chart{height:190px;background:#07101a;border:1px solid var(--line);border-radius:7px;overflow:hidden;position:relative}.chart svg{width:100%;height:100%}.chartlabel{position:absolute;left:7px;top:6px;color:var(--muted);font-size:8px}.candlechart{height:390px;background:#07101a;border:1px solid var(--line);border-radius:7px;overflow:auto;position:relative}.candlechart svg{width:100%;height:100%;min-width:820px}.chartgrid{stroke:#162538;stroke-width:1}.srline{stroke-width:1.5;stroke-dasharray:7 5}.support{stroke:#55d68a}.resistance{stroke:#ff6d7f}.candle-up{fill:#55d68a}.candle-down{fill:#ff6d7f}.wick{stroke-width:1}.analysisbox{background:#0d1725;border:1px solid var(--line);border-radius:8px;padding:8px}.probgrid{display:grid;grid-template-columns:repeat(4,1fr);gap:5px}.prob{background:#111d2d;border:1px solid var(--line);border-radius:7px;padding:7px}.prob span{display:block;color:var(--muted);font-size:8px}.prob b{display:block;margin-top:3px;font-size:12px}.levelrow{display:grid;grid-template-columns:90px 80px 1fr 1fr;gap:6px;align-items:center;padding:7px 5px;border-bottom:1px solid #172436;font-size:9px}.levelrow:last-child{border-bottom:0}.badge{display:inline-block;padding:3px 5px;border-radius:5px;background:#17253a;font-size:8px;font-weight:900}.smallbar{height:5px;background:#162538;border-radius:999px;overflow:hidden}.smallbar i{display:block;height:100%}.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;margin-top:7px}.metric{background:#0d1725;border:1px solid var(--line);border-radius:7px;padding:7px}.metric span{display:block;color:var(--muted);font-size:8px}.metric b{display:block;margin-top:3px;font-size:10px}
...
.bottom{position:fixed;z-index:30;bottom:7px;left:50%;transform:translateX(-50%);width:min(760px,calc(100% - 14px));background:rgba(8,14,24,.97);border:1px solid var(--line);border-radius:10px;padding:4px;display:grid;grid-template-columns:repeat(5,1fr);gap:3px}.bottom button{border:0;background:transparent;color:#8190a5;padding:7px;font-size:8px;font-weight:900;border-radius:7px}.bottom button.active{background:#eaf1fa;color:#07101d}
...
<div class="toolbar"><button class="btn active" data-page="terminal">TERMINAL</button><button class="btn" data-page="chart">CHART & S/R</button><button class="btn" data-page="options">OPTION CHAIN</button><button class="btn" data-page="market">MARKET DATA</button><button class="btn" data-page="signals">SIGNALS</button><button class="btn" data-page="news">NEWS</button></div>
...
<section id="chart" class="page"><div class="panel"><div class="ph"><b>CHART + SUPPORT / RESISTANCE</b><span class="tag">HISTORICAL 5M</span></div><div class="body"><div class="tabs2"><button class="tiny active" onclick="setChartSymbol('NIFTY 50',this)">NIFTY 50</button><button class="tiny" onclick="setChartSymbol('BANK NIFTY',this)">BANK NIFTY</button><button class="tiny" onclick="setChartSymbol('INDIA VIX',this)">INDIA VIX</button></div><div id="chartHeader" class="quote"></div><div id="srChart" class="candlechart"></div><div style="height:7px"></div><div id="probBox" class="analysisbox"></div><div style="height:7px"></div><div id="levelTable" class="analysisbox"></div><div class="note" style="margin-top:7px">Break/hold percentages are model estimates based on historical tests plus current candle, volume and liquidity-proxy context. They are not guaranteed probabilities.</div></div></div></section>
...
<div class="bottom"><button class="active" data-page="terminal">⌂ TERMINAL</button><button data-page="chart">◈ CHART</button><button data-page="options">⌁ OPTIONS</button><button data-page="market">◎ MARKET</button><button data-page="signals">⚡ SIGNALS</button></div>
<script>
...
let live={},options={},macro={},news={},chartData={},selected='NIFTY 50';
...
let chartSymbol='NIFTY 50';
function setChartSymbol(n,el){chartSymbol=n;document.querySelectorAll('#chart .tiny').forEach(x=>x.classList.remove('active'));if(el)el.classList.add('active');renderChartPage();}
function renderSRChart(d){const rows=d.candles||[], levels=d.levels||[];const W=1100,H=370,L=48,R=12,T=18,B=62; if(!rows.length){$('srChart').innerHTML='<div class="mini" style="padding:12px">Historical candles waiting…</div>';return} const vals=rows.flatMap(r=>[Number(r.high),Number(r.low)]);levels.forEach(l=>vals.push(Number(l.price)));let mn=Math.min(...vals),mx=Math.max(...vals);const pad=(mx-mn||1)*.08;mn-=pad;mx+=pad;const x=i=>L+(i/(rows.length-1||1))*(W-L-R);const y=v=>T+(mx-v)/(mx-mn)*(H-T-B);let svg=`<svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none">`;for(let i=0;i<5;i++){const yy=T+i*(H-T-B)/4;svg+=`<line x1="${L}" x2="${W-R}" y1="${yy}" y2="${yy}" class="chartgrid"/>`}const step=Math.max(4,(W-L-R)/rows.length*.62);rows.forEach((r,i)=>{const cx=x(i),yo=y(r.open),yc=y(r.close),yh=y(r.high),yl=y(r.low),up=r.close>=r.open;svg+=`<line x1="${cx}" x2="${cx}" y1="${yh}" y2="${yl}" class="wick ${up?'candle-up':'candle-down'}"/><rect x="${cx-step/2}" y="${Math.min(yo,yc)}" width="${step}" height="${Math.max(1,Math.abs(yc-yo))}" class="${up?'candle-up':'candle-down'}"/>`});const maxVol=Math.max(...rows.map(r=>Number(r.volume||0)),1);rows.forEach((r,i)=>{const cx=x(i),vh=Number(r.volume||0)/maxVol*(B-T-8),vy=H-B+8;svg+=`<rect x="${cx-step/2}" y="${H-B+8+(B-8-vh)}" width="${step}" height="${vh}" opacity=".45" class="${r.close>=r.open?'candle-up':'candle-down'}"/>`});svg+=`<text x="${L}" y="${H-8}" fill="#8190a5" font-size="9">VOLUME</text>`;levels.slice(0,6).forEach(l=>{const yy=y(l.price),cl=l.type==='support'?'support':'resistance';svg+=`<line x1="${L}" x2="${W-R}" y1="${yy}" y2="${yy}" class="srline ${cl}"/><text x="${W-R-4}" y="${yy-3}" text-anchor="end" fill="${l.type==='support'?'#55d68a':'#ff6d7f'}" font-size="10">${l.type==='support'?'S':'R'} ${fmt(l.price)}</text>`});svg+=`<text x="6" y="${T+5}" fill="#8190a5" font-size="9">${fmt(mx)}</text><text x="6" y="${H-B}" fill="#8190a5" font-size="9">${fmt(mn)}</text></svg>`;$('srChart').innerHTML=svg}
function renderChartPage(){const d=chartData[chartSymbol]||{};$('chartHeader').innerHTML=[['LTP',d.price],['CANDLE',d.context?.candle_pattern],['VOLUME',d.context?.volume_ratio==null?null:fmt(d.context.volume_ratio)+'× avg'],['LIQUIDITY',d.context?.liquidity_score==null?null:fmt(d.context.liquidity_score)+'/100'],['PATTERN',d.context?.chart_pattern]].map(q=>`<div class="qbox"><span>${q[0]}</span><b>${q[1]==null?'—':q[1]}</b></div>`).join('');renderSRChart(d);const levels=d.levels||[];const nearest=levels[0];$('probBox').innerHTML=nearest?`<div class="ph" style="margin:-8px -8px 8px"><b>NEAREST ${nearest.type.toUpperCase()}</b><span class="tag">${fmt(nearest.price)}</span></div><div class="probgrid"><div class="prob"><span>BREAK CHANCE</span><b>${fmt(nearest.estimated_break_pct)}%</b></div><div class="prob"><span>HOLD / REJECT</span><b>${fmt(nearest.estimated_hold_pct)}%</b></div><div class="prob"><span>HISTORICAL TESTS</span><b>${nearest.tests??0}</b></div><div class="prob"><span>TOUCHES</span><b>${nearest.touches??0}</b></div></div><div class="note" style="margin-top:7px">Historical break ${fmt(nearest.historical_break_pct)}% • historical rejection ${fmt(nearest.historical_rejection_pct)}% • current context: ${esc(d.context?.candle_pattern||'—')} / ${esc(d.context?.chart_pattern||'—')}</div>`:'<div class="note">Support/resistance levels waiting for enough historical candles.</div>';let h='<div class="ph" style="margin:-8px -8px 6px"><b>SUPPORT / RESISTANCE MAP</b><span class="tag">HISTORICAL TESTS</span></div>';if(!levels.length)h+='<div class="note">No stable swing levels detected yet.</div>';else levels.forEach(l=>{const pct=l.estimated_break_pct??50;h+=`<div class="levelrow"><div><span class="badge ${l.type==='support'?'up':'down'}">${l.type.toUpperCase()}</span></div><b>${fmt(l.price)}</b><div><div class="smallbar"><i style="width:${pct}%"></i></div><div class="mini">Break ${fmt(pct)}%</div></div><div><div class="smallbar"><i style="width:${100-pct}%"></i></div><div class="mini">Hold ${fmt(100-pct)}%</div></div></div>`});$('levelTable').innerHTML=h}
