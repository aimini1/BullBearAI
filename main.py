...
<div class="label">SIGNAL ENGINE • TECHNICAL + OPTIONS</div>
<div id="signals" class="small">Calculating combined signals…</div>
</div>
<div class="card section">
...
async function refresh(){
 try{
  ...
  const signalHtml=Object.entries(d.analysis||{}).map(([name,a])=>{
    const cls=a.bias==='BULLISH'?'green':a.bias==='BEARISH'?'red':'orange';
    return `<div style="padding:8px 0;border-bottom:1px solid #edf0f4">
      <b>${name}</b> • <span class="${cls}"><b>${a.bias||'WAITING'}</b></span>
      <div class="small">Technical ${a.technical_score??0} • Options ${a.option_score??0} • Final ${a.final_score??a.score??0} • Confidence ${a.confidence??0}%</div>
    </div>`;
  }).join('');
  document.getElementById('signals').innerHTML=signalHtml||'Waiting for analysis…';
  ...
  document.getElementById('engine').textContent=ready.length
   ? `LIVE 5M ONLY • EMA20/50 • VWAP • RSI14 • Market Structure • TECH + OPTIONS SIGNAL ENGINE • EMA50 ready ${ema50ok}/${ready.length} • Options ready ${optReady}/${ready.length}`
