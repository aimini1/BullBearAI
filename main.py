...
def update_from_feed(feed):
    ...
    for name, key in INSTRUMENTS.items():
        item = feeds.get(key)
        if not item:
            # Some payloads use colon-separated instrument keys.
            item = feeds.get(key.replace("|", ":"))
        if not item:
            continue
        ...
        # Upstox V3 full-feed depth is normally under marketFF.marketLevel.bidAskQuote.
        # Keep parsing defensive so a feed-shape change does not break LTP updates.
        level = market_ff.get("marketLevel") or full.get("marketLevel") or {}
        ...
        if isinstance(quotes, list):
            for q in quotes:
                try:
                    ...
                    continue
        market_depth[name] = {
            "bids": bids[:5], "asks": asks[:5], "updated_at": now_ist()
        }
...
function renderDepth(){const d=live.market_depth?.[selected]||{};$('depthSym').textContent=selected;$('bids').innerHTML=(d.bids||[]).map(x=>`<div class="depthrow bid"><b>${fmt(x.qty)}</b><span>${fmt(x.price)}</span></div>`).join('')||'<div class="mini">Depth waiting…</div>';$('asks').innerHTML=(d.asks||[]).map(x=>`<div class="depthrow ask"><span>${fmt(x.price)}</span><b>${fmt(x.qty)}</b></div>`).join('')||'<div class="mini">Depth waiting…</div>'}
